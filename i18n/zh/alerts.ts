...
export const alerts: typeof English = {
  ...
  housekeepingBody: (months) =>
    `应用每次打开都会读取你的全部记录，所以超过 ${months ?? ''} 个月的记录会被清理，让应用保持流畅。现在还没有删除任何东西。先到「报表 → 月结单」把那几个月保存下来，或者选择保留更久。你的余额不会受影响。`,
  dividendBody: (units) =>
    `按你在除权日持有的 ${units} 股计算，和其他存入一样分到你的钱罐。公司会扣除税费，请核对实际到账的金额，如果不一样，到「交易」里修改。`,
